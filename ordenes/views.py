...
@solo_mesero
def marcar_orden_entregada(request, orden_id):
    """
    Permite a un mesero marcar una orden como 'entregada'.
    """
    orden = get_object_or_404(Orden, id=orden_id)
    if request.method == 'POST':
        # Solo permitir marcar como entregada si no está ya entregada o cancelada
        if orden.estado not in ['entregado', 'cancelado']:
            orden.estado = 'entregado'
            orden.save()
            messages.success(request, f"¡Orden #{orden.id} marcada como entregada exitosamente!")
        else:
            messages.warning(request, f"La orden #{orden.id} ya ha sido entregada o cancelada y no puede modificarse.")
    return redirect('ordenes:detalle_orden', orden_id=orden.id)

@solo_mesero
def cancelar_orden(request, orden_id):
    """
    Permite a un mesero cancelar una orden.
    """
    orden = get_object_or_404(Orden, id=orden_id)
    if request.method == 'POST':
        orden.estado = 'cancelado'
        orden.save()
        messages.success(request, f"¡Orden #{orden.id} ha sido cancelada!")
    return redirect('ordenes:detalle_orden', orden_id=orden.id)
